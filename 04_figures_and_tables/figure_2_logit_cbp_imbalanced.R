...
data_use <- read.csv(file = "../03_postprocessing/logit_result_data/logit_imbalanced_sim500_boot100.csv")
...
for(i in 1:nrow(design_table)){
  ...
  # TRUE_coef
  true_d <- data_use[data_use$design_num == i & data_use$estimator == "true_lo", ] 
  true_coef_mat[i, 1:4] <- true_coef <- apply(true_d[, coef_name], 2, mean)
  
  # SO
  res_Pro <- data_use[data_use$design_num == i & data_use$estimator == "res_Pro_lo", ] 
  ...
  # GSO
  res_IPW <- data_use[data_use$design_num == i & data_use$estimator == "res_IPW_lo", ] 
  ...
  # SSL
  res_PI <- data_use[data_use$design_num == i & data_use$estimator == "res_PI_lo", ] 
  ...
  # DSL
  res_dm <- data_use[data_use$design_num == i & data_use$estimator == "res_dm_lo", ] 
...
pdf("figure/Figure2_CBP_imbalanced_zero_shot.pdf", height = 3, width = 9)
par(mfrow = c(1,3))
plot(use_x, PO_b[use_index], ylim = c(0, 1.5),
...
plot(use_x, log(PO_RMSE[use_index]), ylim = c(-2.0, 1.5),
...
Axis(side = 2, at = seq(from = -2, to = 1.5, by = 0.5), 
     labels = round(exp(seq(from = -2, to = 1.5, by = 0.5)), 2))
dev.off()

# Imbalanced Five-Shot 
use_index <- c(7:10)
x_use <- design_table$read[use_index]
use_x <- c(1:4)

pdf("figure/Figure2_CBP_imbalanced_five_shot.pdf", height = 3, width = 9)
par(mfrow = c(1,3))
plot(use_x, PO_b[use_index], ylim = c(0, 1.5),
     ylab = "Standardized Bias", xlab = "Size of Gold-Standard Data", 
     xaxt='n', main = "Bias", pch = 19, col = "red", type = "b", lwd = 2)
...
plot(use_x, log(PO_RMSE[use_index]), ylim = c(-2.5, 1.5),
     ylab = "RMSE", xlab = "Size of Gold-Standard Data", 
     xaxt='n', main = "RMSE", pch = 19, col = "red", type = "b", lwd = 2, yaxt='n')
Axis(side = 1, at = use_x, labels = x_use)
points(use_x, log(GSO_RMSE[use_index]), pch = 17, col = "green", type = "b", lwd = 2)
points(use_x, log(ssl_RMSE[use_index]), pch = 23, col = "orange", bg = "orange", type = "b", lwd = 2)
points(use_x, log(dsl_RMSE[use_index]), pch = 15, col = "blue", type = "b", lwd = 2)
Axis(side = 2, at = seq(from = -2.5, to = 1.5, by = 0.5), 
     labels = round(exp(seq(from = -2.5, to = 1.5, by = 0.5)), 2))
dev.off()
